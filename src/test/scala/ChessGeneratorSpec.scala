...
class ChessGeneratorSpec extends FlatSpec with Matchers {
  val ast = ChessCompiler(
    """
      | piece "Bishop"(B) with [
      |   Move:Line,
      ...
      | |_|_|_|
      | |b|_|_|
      |""".stripMargin.trim).right.getOrElse(null)
  ...
  "Chess compiler" should "successfully parse a valid chess setup" in {
    val generatedFiles = ChessGenerator(ast);
